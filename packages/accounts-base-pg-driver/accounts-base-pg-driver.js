AccountsDBClientPG = class AccountsDBClientPG {
    ...
    _getUserWhere(where) {
        ...
        fullUserDoc.emails = _.map(emailRows, function (row) {
            return {
                address: row.address,
    ...
    _getResumeService(userId) {
        ...
        const formattedLoginTokens = loginTokens.map(function (loginToken) {
            return {
    ...
    _getServiceData(userId) {
        ...
        const serviceData = {};
        serviceDataRows.forEach(function (row) {
    ...
    insertUserDoc(fullUserDoc) {
        ...
        PG.inTransaction(function () {
            ...
            if (!_.isEmpty(fullUserDoc.emails)) {
                fullUserDoc.emails.forEach(function (email) {
                    email = _.clone(email);
            ...
            if (!_.isEmpty(fullUserDoc.services)) {
                Object.keys(fullUserDoc.services).forEach(function (serviceName) {
    ...
    _insertServiceRecords(serviceName, serviceData, userId) {
        const serviceRecords = Object.keys(serviceData).map(function (key) {
